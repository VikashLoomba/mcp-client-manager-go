...
function TableHeader({ className, ...props }: React.ComponentProps<"thead">) {
  return (
    <thead
      ...
      className={cn("[&_tr]:border-b [&_tr]:hairline bg-white/18 dark:bg-white/10", className)}
      {...props}
    />
  )
}
...
function TableRow({ className, ...props }: React.ComponentProps<"tr">) {
  return (
    <tr
      data-slot="table-row"
      className={cn(
        "border-b hairline transition-colors bg-white/12 dark:bg-white/6 hover:bg-white/28 dark:hover:bg-white/12 data-[state=selected]:bg-white/45 dark:data-[state=selected]:bg-white/18",
        className
      )}
      {...props}
    />
